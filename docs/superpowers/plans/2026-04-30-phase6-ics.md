...
async def _seed_user_with_token(
    ...
    if with_segment:
        trip = Trip(
            title="Paris", start_date=date(2026, 6, 1), end_date=date(2026, 6, 7), created_by=u.id
        )
        db.add(trip)
        await db.flush()
...
@pytest.mark.asyncio
async def test_invalid_token_returns_404(db_url: str, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("DATABASE_URL", db_url)
    settings = Settings()
    async with _client(settings) as c:
