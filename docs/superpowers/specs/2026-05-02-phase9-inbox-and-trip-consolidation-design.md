...
| 4 | Filter clause `WHERE merged_into_id IS NULL` missed in some Trip query | Codebase grep audit during implementation; the plan task list itemizes every Trip select. Partial index `ix_trips_active` makes the active-only path the cheap one. |
